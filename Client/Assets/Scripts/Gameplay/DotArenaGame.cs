...
public sealed class DotArenaGame : MonoBehaviour, IPlayerCallback
{
    ...
    [SerializeField] private RpcEndpointSettings _endpoint = RpcEndpointSettings.CreateWebSocket("127.0.0.1", 20000);
    ...
    protected RpcEndpointSettings Endpoint => _endpoint;
    ...
    private void OnGUI()
    {
        ...
        GUI.Label(new Rect(contentRect.x, contentRect.y + 44f, contentRect.width, 18f),
            $"账号: {_account}   地址: {_host}:{_port}", bodyStyle);
        ...
        GUI.Label(new Rect(contentRect.x, contentRect.y + 104f, contentRect.width, 18f), "W/A/S/D 只发送移动指令，位置由服务器推送。",
            bodyStyle);
    }

    public void OnMove(PlayerPositions playerPositions)
    {
        var snapshot = new List<PlayerPosition>(playerPositions.playerPositions.Count);
        foreach (var playerPosition in playerPositions.playerPositions)
